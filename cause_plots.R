### Plot under-5 mortality and interventions

### Setup
library(data.table); library(ggplot2); library(RColorBrewer); library(ggrepel); library(shiny)
...
## Paths
data.path <- "C:/Users/AustinC/OneDrive - bgC3/Documents/data/U5M/GBD16_results_all2.csv"
policy.path <- "C:/Users/AustinC/OneDrive - bgC3/Documents/data/U5M/ebi_policies.csv"
ebi.path <- "C:/Users/AustinC/bgC3/EGH - Content Development - Under-5 Mortality/Exemplar countries/20180731_STAT Compiler All Countries (Prepped).csv"
...
ebi.dt <- fread(ebi.path)[!is.na(value)]
...
start.val <- mort.dt[metric == "Rate" & age == "Under 5" & year == 2000 & measure == "Deaths" & location == cloc, .(cause, val)]
setnames(start.val, "val", "start_val")
end.val <- mort.dt[metric == "Rate" & age == "Under 5" & year == 2015 & measure == "Deaths" & location == cloc, .(cause, val)]
...
subset.mort[, cast_val := ifelse(year == 2000, lower, upper)]
cast.dt <- dcast(subset.mort, location + cause ~ year, value.var = "cast_val")
cast.dt[, sig := ifelse(get("2000") > get("2015"), 1, 0)]
cast.dt[is.na(sig), sig := 0]

### Plots
## All cause with EBI Timeline
for(cloc in unique(mort.dt$location)) {
  print(cloc)
  if(bar) {
  ...
  pdf(plot.path, width = 11, height = 8.5)
  ...
  # Set cause order
  cause.order <- copy(subset.mort[year == 2000, .(cause, val)])
  ...
  if(all.plots) {
    ...
    if(nrow(subset.policy) > 0) {
      ...
      gg <- gg + geom_segment(data = policy.dt, aes(x = start_year, y = 0, xend = start_year, yend = policy.y - 5), linetype = "dashed") +
        geom_text(data = policy.dt, aes(x = start_year, y = policy.y, label = policy_name), size = 3, hjust = "left", nudge_x = 0.1)
    }
    # Format
    gg <- gg + ggtitle(paste0(cage, " mortality in ", cloc, ": ", abs(decline), "% ",ifelse(decline > 0, "decline", "increase"), " 2000-2015",  ifelse(sig == 1, "*", ""))) + ylab(paste(cmeasure, cmetric)) + xlab("Year") + labs(fill = "Cause") + theme_bw() + 
      theme(text = element_text(size=15), legend.position = "bottom") + expand_limits(y = 1.2 * max(subset.mort$val)) +
  ...
  if(cause.plots) {
    ...
    for(ccause in setdiff(causes, "All causes")) {
      ...
      gg <- gg + ggtitle(paste0(ccause, " mortality in ", cloc, ": ", abs(decline), "% ", ifelse(decline > 0, "decline", "increase"), " 2000-2015",  ifelse(sig == 1, "*", ""))) + 
        ylab(paste(cmeasure, cmetric)) + xlab("Year") + labs(fill = "Cause") + theme_bw() + 
        theme(text = element_text(size=15), legend.position = "bottom") + expand_limits(y = c(0, max.val)) +
